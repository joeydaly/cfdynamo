...
<cfcomponent output="false" hint="The ColdBox Mail Service used to send emails in an oo and ColdBoxy fashion">
	...
	<cffunction name="init" access="public" output="false" returntype="MailService" hint="Constructor">
		<cfargument name="mailSettings" type="any" required="false" hint="A configured mail settings bean with default mail configurations, else ignored and uses payload" colddoc:generic="mockbox.system.core.mail.MailSettingsBean"/>
	...
	<!--- Mail Settings --->
	<cffunction name="getMailSettingsBean" output="false" access="public" returntype="any" hint="Get the mail settings configuration object" colddoc:generic="mockbox.system.core.mail.MailSettingsBean">
		<cfreturn variables.mailSettings>    	
    </cffunction>

	<!--- newMail --->
	<cffunction name="newMail" access="public" returntype="any" output="false" hint="Get a new Mail payload object, just use config() on it to prepare it or pass in all the arguments via this method" colddoc:generic="mockbox.system.core.mail.Mail">
		<cfscript>
			var mail 		 = createObject("component","mockbox.system.core.mail.Mail").init(argumentCollection=arguments);
	...
	<cffunction name="send" access="public" returntype="struct" output="false" hint="Send an email payload. Returns a struct: [error:boolean,errorArray:array]">
		<cfargument name="mail" required="true" type="any" hint="The mail payload to send." colddoc:generic="mockbox.system.core.mail.Mail"/>
		<cfscript>
	...
	<cffunction name="parseTokens" access="private" returntype="void" output="false" hint="Parse the tokens and do body replacements.">
		<cfargument name="mail" required="true" type="any" hint="The mail payload" colddoc:generic="mockbox.system.core.mail.Mail"/>
		<cfscript>
